...
{
    $searchedFor = "%$search%";
    $statement = $database->prepare('SELECT lists.id, title, tasks.id AS task_id, tasks.user_id, tasks.list_id, tasks.task, tasks.description,
    tasks.deadline, tasks.completed FROM tasks LEFT JOIN lists ON tasks.list_id = lists.id WHERE task LIKE :search OR description LIKE :search OR title LIKE :search AND tasks.user_id = :user_id');
